...
The RBAC seed script has **dependency phases** (permissions → roles → groups → admin user). Parallelize **within** each phase but keep phases sequential:
...
# Phase 3: groups (depend on roles, but independent of each other)
group_results = await asyncio.gather(*[create_group(g, role_map) for g in GROUPS])

# Phase 4: admin user (depends on groups)
await create_admin_user(admin_def, group_map)
```

For batch inserts of independent records:
