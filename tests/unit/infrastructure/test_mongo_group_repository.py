...
class TestGroupSave:
    async def test_insert_new_group_sets_id(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
    ...
    async def test_replace_existing_group(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
...
class TestGroupFind:
    async def test_find_by_id_returns_group(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
    ...
    async def test_find_by_id_returns_none(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
    ...
    async def test_find_by_name_returns_group(
        self,
        ...
        result = await repository.find_by_name("Admins")
    ...
    async def test_find_all(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
        docs = [
    ...
    async def test_find_by_ids(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
        docs = [
            {"_id": "1", "name": "Admins", "description": "Admin group", "role_ids": []},
        ]
        mock_collection.find.return_value.to_list = AsyncMock(return_value=docs)
    ...
    async def test_find_by_app_id_returns_matching_groups(
        ...
        result = await repository.find_by_app_id("hhh-frontend")
    ...
    async def test_find_by_app_id_returns_empty_when_no_match(
        self, repository: MongoGroupRepository, mock_collection: MagicMock
    ) -> None:
        mock_collection.find.return_value.to_list = AsyncMock(return_value=[])

        result = await repository.find_by_app_id("unknown-app")

        assert result == []


class TestGroupDelete:
    async def test_delete_returns_true(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
        mock_collection.delete_one = AsyncMock(return_value=MagicMock(deleted_count=1))

        assert await repository.delete("507f1f77bcf86cd799439011") is True

    async def test_delete_returns_false(self, repository: MongoGroupRepository, mock_collection: MagicMock) -> None:
