...
class TestCreatePermission:
    async def test_creates_and_returns_permission(
        ...
        result = await service.create_permission("auth:users:read", "Read users")
...
class TestGetPermission:
    async def test_returns_permission_when_found(
        ...
        result = await service.get_permission("p-1")

        assert result.id == "p-1"

    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_permission_repository: MagicMock) -> None:
        mock_permission_repository.find_by_id.return_value = None

        with pytest.raises(PermissionNotFoundError):
            await service.get_permission("missing")


class TestListPermissions:
    async def test_returns_all_permissions(
        self,
...
class TestUpdatePermission:
    async def test_updates_and_returns_permission(
        self, service: RbacServiceImpl, mock_permission_repository: MagicMock
    ) -> None:
        ...
        result = await service.update_permission("p-1", "new", "New")
    ...
    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_permission_repository: MagicMock) -> None:
        ...
        with pytest.raises(PermissionNotFoundError):
            await service.update_permission("missing", "x", "X")


class TestDeletePermission:
    async def test_deletes_successfully(self, service: RbacServiceImpl, mock_permission_repository: MagicMock) -> None:
        mock_permission_repository.delete.return_value = True

        await service.delete_permission("p-1")
...
class TestCreateRole:
    async def test_creates_and_returns_role(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        ...
        result = await service.create_role("Admin", "Admin role", ["p-1", "p-2"])
...
class TestGetRole:
    async def test_returns_role_when_found(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        mock_role_repository.find_by_id.return_value = Role(id="r-1", name="Admin")

        result = await service.get_role("r-1")

        assert result.id == "r-1"

    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        mock_role_repository.find_by_id.return_value = None

        with pytest.raises(RoleNotFoundError):
            await service.get_role("missing")


class TestListRoles:
    async def test_returns_all_roles(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
...
class TestUpdateRole:
    async def test_updates_and_returns_role(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        ...
        result = await service.update_role("r-1", "New", "New desc", ["p-2"])
    ...
    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        ...
        with pytest.raises(RoleNotFoundError):
            await service.update_role("missing", "x", "x", [])


class TestDeleteRole:
    async def test_deletes_successfully(self, service: RbacServiceImpl, mock_role_repository: MagicMock) -> None:
        mock_role_repository.delete.return_value = True

        await service.delete_role("r-1")
...
class TestCreateGroup:
    async def test_creates_and_returns_group(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        mock_group_repository.save.side_effect = lambda g: Group(
            id="g-1", name=g.name, description=g.description, role_ids=g.role_ids
        )

        result = await service.create_group("Admins", "Admin group", ["r-1"])

        assert result.id == "g-1"
        assert result.name == "Admins"
        assert result.role_ids == ["r-1"]

    async def test_creates_group_with_auto_assign_apps(
        self, service: RbacServiceImpl, mock_group_repository: MagicMock
    ) -> None:
        mock_group_repository.save.side_effect = lambda g: g

        result = await service.create_group("Users", "Users group", ["r-1"], auto_assign_apps=["hhh-frontend"])

        assert result.auto_assign_apps == ["hhh-frontend"]

    async def test_creates_group_without_auto_assign_apps_defaults_to_empty(
        self, service: RbacServiceImpl, mock_group_repository: MagicMock
    ) -> None:
        mock_group_repository.save.side_effect = lambda g: g
...
class TestGetGroup:
    async def test_returns_group_when_found(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        mock_group_repository.find_by_id.return_value = Group(id="g-1", name="Admins")

        result = await service.get_group("g-1")

        assert result.id == "g-1"

    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
...
class TestListGroups:
    async def test_returns_all_groups(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
...
class TestUpdateGroup:
    async def test_updates_and_returns_group(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        existing = Group(id="g-1", name="Old", description="Old", role_ids=["r-1"])
        mock_group_repository.find_by_id.return_value = existing
        mock_group_repository.save.side_effect = lambda g: g

        result = await service.update_group("g-1", "New", "New desc", ["r-2"])

        assert result.name == "New"
        assert result.role_ids == ["r-2"]

    async def test_updates_group_with_auto_assign_apps(
        self, service: RbacServiceImpl, mock_group_repository: MagicMock
    ) -> None:
        ...
        result = await service.update_group("g-1", "Users", "Users", ["r-1"], auto_assign_apps=["hhh-frontend"])

        assert result.auto_assign_apps == ["hhh-frontend"]

    async def test_updates_group_without_auto_assign_apps_preserves_existing(
        self, service: RbacServiceImpl, mock_group_repository: MagicMock
    ) -> None:
        existing = Group(
        ...
        result = await service.update_group("g-1", "Users", "Updated desc", ["r-1"])
    ...
    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        ...
        with pytest.raises(GroupNotFoundError):
            await service.update_group("missing", "x", "x", [])


class TestDeleteGroup:
    async def test_deletes_successfully(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        mock_group_repository.delete.return_value = True

        await service.delete_group("g-1")
    ...
    async def test_raises_when_not_found(self, service: RbacServiceImpl, mock_group_repository: MagicMock) -> None:
        ...
        with pytest.raises(GroupNotFoundError):
            await service.delete_group("missing")
...
class TestResolvePermissions:
    async def test_resolves_user_group_role_permission_chain(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
        ...
        result = await service.resolve_permissions("u-1")

        assert result == ["auth:users:read", "auth:users:write"]

    async def test_deduplicates_permissions_from_overlapping_roles(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_returns_empty_list_when_groups_have_no_roles(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_returns_empty_list_when_roles_have_no_permissions(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_raises_when_user_not_found(
        ...
        with pytest.raises(UserNotFoundError):
            await service.resolve_permissions("missing")
...
class TestAssignUserToGroup:
    async def test_adds_group_id_to_user(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
        ...
        await service.assign_user_to_group("u-1", "g-1")

        assert "g-1" in user.group_ids
        mock_user_repository.save.assert_called_once()

    async def test_does_not_duplicate_group_id(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_raises_when_user_not_found(
        ...
        with pytest.raises(UserNotFoundError):
            await service.assign_user_to_group("missing", "g-1")

    async def test_raises_when_group_not_found(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
        ...
        with pytest.raises(GroupNotFoundError):
            await service.assign_user_to_group("u-1", "missing")


class TestRemoveUserFromGroup:
    async def test_removes_group_id_from_user(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
    ) -> None:
        ...
        await service.remove_user_from_group("u-1", "g-1")

        assert "g-1" not in user.group_ids
        assert "g-2" in user.group_ids
        mock_user_repository.save.assert_called_once()

    async def test_no_op_when_group_id_not_present(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
    ) -> None:
    ...
    async def test_raises_when_user_not_found(
        ...
        with pytest.raises(UserNotFoundError):
            await service.remove_user_from_group("missing", "g-1")
...
class TestResolveRbacClaims:
    async def test_resolves_full_claims(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_returns_groups_only_when_no_roles(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_returns_groups_and_roles_when_no_permissions(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_deduplicates_roles_and_permissions(
        self,
        service: RbacServiceImpl,
        mock_user_repository: MagicMock,
        mock_group_repository: MagicMock,
    ...
    async def test_raises_when_user_not_found(
        ...
        with pytest.raises(UserNotFoundError):
            await service.resolve_rbac_claims("missing")
